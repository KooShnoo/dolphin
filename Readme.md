...
## this is KooShnoo's fork
I'm breaking things atm. As of right now the big thing I added is a "FrameHeatMap", which allows you to get a report
of heat data. It gives organized, frame by frame, per-function heat data. It's a work in progress with work currently
on hold. Current problems include: 
- it's actually analyzing JIT blocks, not functions
- the data output is formatted abysmally
- probably lots more that i dont even know

if you are for some reason curious about it, my discord is `kooshnoo`. DMs open for folks in the dolphin discord.
